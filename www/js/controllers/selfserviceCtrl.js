...
.controller('SelfServiceCtrl',function($scope,$cordovaNetwork,$ionicPopup,WorkFlowService,$filter){
...
.controller('SelfServiceListCtrl',function($scope,$stateParams,$cordovaNetwork,$ionicPopup,$filter,WorkFlowService){
...
.controller('CreateRedeemDutyCtrl',function($scope,$q,ionicDatePicker,APIService,$ionicPopup,$cordovaNetwork,$location,WorkFlowService){
	$scope.noInternet = false;
			...
			if(response){
				...
				if(confirm(message)){
					...
					WorkFlowService.CreateWorkFlow(data).then(function(response){
						if(response != null) RedirectAndReloadView('#/app/selfservicelist/1');
...
.controller('ItemRedeemDutyCtrl',function($scope,$ionicPopup,$cordovaNetwork,$stateParams,WorkFlowService){
    ...
    if(!CheckNetwork($cordovaNetwork)) OpenIonicAlertPopup($ionicPopup,'ไม่มีสัญญานอินเตอร์เนท','ไม่สามารถใช้งานส่วนนี้ได้เนื่องจากไม่ได้เชื่อมต่ออินเตอร์เนท');
    	...
    	WorkFlowService.ViewDutyChangeStaff($scope.documentId).then(function(response){
    		if(response != null && response.data != null){
    			...
    			//bind all history
    			WorkFlowService.ViewHistoryMyTask($scope.documentId,$scope.nextLevel,window.localStorage.getItem("CurrentUserName")).then(function(response){
    				if(response != null && response.data != null){
    ...
    $scope.doAcknowledge = function(){
    	if(confirm('รับทราบรายการนี้')){
    		WorkFlowService.AcknowledgmentWorkflow($scope.documentId,window.localStorage.getItem("CurrentUserName"),$scope.action.remark).then(function(response){
    			RedirectAndReloadView('#/app/selfservicelist/1');
    		});
    	}
    };
